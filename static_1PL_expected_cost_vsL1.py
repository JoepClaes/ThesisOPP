...
rho_u_under = underline('ρᵤ') 

def overline(text):
    return ''.join(c + '\u0305' for c in text)

rho_u_over = overline('ρᵤ')  

# Plot the results
plt.figure(figsize=(10, 6))
for rho_u in rho_u_values:
    if rho_u == rho_u_values[0]:
        plt.plot(L1_values, costs[rho_u], label=r"$\rho_u$" + " = " + f"{rho_u_under}")
    elif rho_u == rho_u_values[-1]:
        plt.plot(L1_values, costs[rho_u], label=r"$\rho_u$" + " = " + f"{rho_u_over}")
    else:
        plt.plot(L1_values, costs[rho_u], label=fr"$\rho_u$ = {rho_u:.3f}")
