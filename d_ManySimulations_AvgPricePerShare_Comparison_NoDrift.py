...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 20 11:07:42 2025
...
# Simulation function for no-drift strategy
...
# NEW: Array to store terminal average prices for no-drift strategy
avg_price_nodrift_T = np.zeros(n_sim)

# Run simulations NODRIFT
for sim_index in range(n_sim):
    prices_sim = prices[:, sim_index]  # FIX: Define prices_sim here
    # NODRIFT strategy
    ...
    # NEW: Store terminal average price
    avg_price_nodrift_T[sim_index] = avg_price_NODRIFT[-1]

# NEW: Print comparison of terminal average prices
#print("\nTerminal Average Price Comparison:")
#for sim_index in range(n_sim):
#    print(f"Simulation {sim_index + 1}: Optimal strat. average price = ${avg_price_opt_T[sim_index]:.3f} vs. No-drift price = ${avg_price_nodrift_T[sim_index]:.3f}")

total_avg_price_opt_T = np.mean(avg_price_opt_T)
total_avg_price_nodrift_T = np.mean(avg_price_nodrift_T)

print("\nAverage Average Price across all simulations Comparison:")
print(f"Optimal strategy total average price = ${total_avg_price_opt_T:.3f} vs. No-drift strtagey total average price = ${total_avg_price_nodrift_T:.3f}")
