...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 16 16:54:37 2025

@author: jawclaes
"""

# THIS SCRIPT PLOTS THE OPTIMAL DEPTHS AND OPTIMAL STOPPING TIMES FOR ANY INVENOTRY SIZE
...
for i in range(1,R+1):
    print("For inventory "+ str(i) + " we have stopping time " +str(tau(i)))

check1 = mu + lam/kappa - f_theta(1,theta)
check2 = mu + lam/kappa + f_theta(1,theta) - f_theta(2,theta)

mu_stopping = [-0.0003, -0.0002, -0.0001, 0,  0.0001, 0.0002, 0.0003]
...
for S in range(R):
    ...
    A_S_1 = A(S - 1, kappa, mu, theta)
    denominator = A_S - A_S_1 + lam
    print(lam/denominator)

# This figure plits
plt.figure(figsize=(10, 6))
plt.plot(tau_inv, inventory, 'o', label='Stopping Times')
plt.plot(time,twap_schedule, '--', label='TWAP')
plt.title("Optimal stopping time at different inventories")
plt.xlabel('Time (t)')
